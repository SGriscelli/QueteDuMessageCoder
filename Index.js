...
const index = hiddenMessage.indexOf("hidden"); 
console.log (index)
while (hiddenMessage.indexOf("X") !== -1) {
    const index = hiddenMessage.indexOf("X");
...
const codetab = hiddenCode.split("");

while (codetab.indexOf("X") !== -1) {
  const index = codetab.indexOf("X");
  codetab.splice(index, 1);
}

let i = 0;
while (i < codetab.length) {
  if (!Number.isNaN(codetab[i]) && Number.parseInt(codetab[i]) % 1 === 0) {
    codetab.splice(i, 1);
  } else {
    i++;
  }
